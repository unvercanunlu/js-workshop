const checkListValid = (list) => {
  return list !== undefined && list !== null && Array.isArray(list);
};

const checkNumberInvalid = (value) => {
  return (
    value === undefined ||
    value === null ||
    typeof value !== 'number' ||
...
const sortListAscending = (list) => {
  return [...list].filter((x) => !checkNumberInvalid(x)).sort((a, b) => a - b);
};
